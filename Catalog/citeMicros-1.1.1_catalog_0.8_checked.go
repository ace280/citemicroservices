...
//maybe there should be some overall description of what the programm does here?
...
//Stores text version information which is parsed to JSON format and displayed. Used in ReturnTextsVersion
...
//Splits CTS string into its Stem and Reference. Returns CTSURN.
...
//Loads and parses JSON file defined by file. Returns ServerConfig.
...
//Returns bool for whether string slice stringSlice contains testedString //isn't this a clone of the standard strings pkg's Contains function?
...
func isRange(s string) bool {
	...
	case strings.Contains(strings.Split(s, ":")[4], "-"): //The Reference must contain a hyphen ('-') indicating a range of URNs.
		return true //Then it should be a range.
	default: //In any other case..
		return false //...it's not a CTS URN range.
	}
}
...
//Returns bool for whether string e is contained in string slice s on level 1 of the URN. //Even with the comments I don't understand at first glace what these functions do.
...
//Initializes mux server, loads configuration from config file, sets the serverIP, maps endpoints to respective funtions. Initialises the headers. Sets up logging.
func main() {
...
//Parses the text in order to extract the URNS from #!ctsdata. Returns URNResponse. Called by ReturnWorkURNS //This func need comments
...
func ParseURNsAndTextsFromCTSdata(ctsParams CTSParams) Work {
	...
	reader.LazyQuotes = true                        //what does this do?
...
func ReturnCiteVersion(w http.ResponseWriter, r *http.Request) {
	...
	log.Println("ReturnCiteVersion executed succesfully") //should be the last line in the function...
...
func ReturnWorkURNS(w http.ResponseWriter, r *http.Request) {
	defer func() { //Recover fuction for error handling //I find this defered function difficult to understand but perhaps that's just because I'm unfamiliar with such functions
	...
	urnResponse := ParseURNSFromCTSdata(CTSParams{Sourcetext: sourcetext}) //could do with more comments here
...
func ReturnFirst(w http.ResponseWriter, r *http.Request) { //see my comments on the ReturnWorkURNS functions above
...
//Returns next entry of URN range corresponding to given URN. //These longer functions could do with more comments
...
func ReturnReff(w http.ResponseWriter, r *http.Request) {
	...
	requestURN := vars["URN"]         //save requested URN
...
//a general comment here is missing
func ReturnCatalog(w http.ResponseWriter, r *http.Request) {
	...
	//it would be helpful to have comments next to the switch case statements explaining what they're testing for, even though it's relatively self explanatory :w
